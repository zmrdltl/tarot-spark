...
Use this rule to choose the lightest process that still protects code and document quality.

## Default Path

- Do not create a GitHub Issue or PR by default.
- For small, clear, single-owner work, keep the scope in the current task, make the change, run the relevant quality gates, and summarize the result.
...
- The work affects product direction, monetization, analytics, SEO, policy, or another area where decisions should be revisited later.
...
- Documentation rules apply to contributor-facing docs, README changes, PR text, and public copy.
- Secrets, private identifiers, generated-by notes, and contributor-irrelevant internal notes must stay out of committed files.
