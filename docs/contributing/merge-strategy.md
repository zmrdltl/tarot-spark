...
- Use "Rebase and merge" when repository settings allow it and the PR commits are ready to preserve.
- Use "Squash and merge" when the branch contains noisy fixup commits that should become one commit.
...
- Confirm issue-closing keywords are present only when the issue should close on merge.
...
- Create follow-up tasks or issues for remaining work instead of reopening completed PR scope.
